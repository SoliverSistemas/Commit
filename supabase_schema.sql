...
-- Tabela de Paletas Pré-definidas (gerenciadas pelo MASTER)
create table if not exists paletas_predefinidas (
    id uuid primary key default gen_random_uuid(),
    nome text not null,
    slug text not null unique,
    descricao text not null default '',
...
create index if not exists idx_paletas_predefinidas_ativo on paletas_predefinidas(ativo, sort_order);

drop trigger if exists trg_paletas_predefinidas_updated_at on paletas_predefinidas;
create trigger trg_paletas_predefinidas_updated_at before update on paletas_predefinidas
for each row execute function set_updated_at();

-- Adicionar coluna cor_texto_escuro na tabela pizzarias (para textos escuros como títulos de categoria)
alter table pizzarias add column if not exists cor_texto_escuro text not null default '#1f2937';

-- Tabela de subcategorias (criadas por cada pizzaria)
create table if not exists subcategorias (
    id bigint generated by default as identity primary key,
    pizzaria_id bigint references pizzarias(id) on delete cascade,
    categoria_global_id bigint references categorias_globais(id) on delete cascade,
    nome text not null,
...
create index if not exists idx_subcategorias_pizzaria on subcategorias(pizzaria_id, categoria_global_id);

-- Tabela de seções de produtos (configurador)
create table if not exists produto_secoes (
    id bigint generated by default as identity primary key,
    ...
    tipo text not null default 'single' check (tipo in ('single', 'multiple')),
    obrigatorio boolean not null default false,
    max_selecao integer, -- null = ilimitado
    ordem integer not null default 0,
    created_at timestamptz not null default now(),
    updated_at timestamptz not null default now()
);
create index if not exists idx_produto_secoes_produto on produto_secoes(produto_id);

-- Tabela de opções de seções
create table if not exists produto_opcoes (
    id bigint generated by default as identity primary key,
    secao_id bigint references produto_secoes(id) on delete cascade,
    nome text not null,
...
create index if not exists idx_produto_opcoes_secao on produto_opcoes(secao_id);

-- Adicionar colunas na tabela produtos
alter table produtos add column if not exists subcategoria_global_id bigint references subcategorias(id);
alter table produtos add column if not exists foto text;
alter table produtos add column if not exists secoes jsonb default '[]'::jsonb;
